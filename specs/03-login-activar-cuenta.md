...
4. Crear `app/components/ActivateForm.tsx`: valores iniciales del mockup (código "7K4P9", email de Lucía, contraseña "contraseña", checkbox marcado), validación (todos requeridos excepto checkbox, formato email), submit válido → `router.push("/")`.
...
- **No:** checkbox de consentimiento obligatorio — es opcional; el usuario decidió que no debe bloquear la activación de cuenta.
